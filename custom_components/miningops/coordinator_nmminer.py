"""NMMiner coordinator for Mining Ops integration."""
...
class NMMinerDataCoordinator(DataUpdateCoordinator):
    """Coordinator for NMMiner UDP broadcast data."""
    ...
    def __init__(self, hass: HomeAssistant, port: int) -> None:
        ...
        super().__init__(
            ...
            _LOGGER,
            name=DOMAIN,
            update_interval=timedelta(seconds=NMMINER_UPDATE_INTERVAL),
        )
        self.port = port
    ...
    def async_process_miner_data(self, miner_ip: str, data: dict[str, Any]) -> None:
        ...
        if miner_ip in self.miners:
            ...
            if new_valid > old_valid:
                _LOGGER.info(
                    "Block found on miner %s! Valid blocks: %s", miner_ip, new_valid
                )
