...
A unified Home Assistant custom integration for monitoring Bitcoin mining operations. Supports both ESP32-based miners (NMMiner firmware) and Rigol Bitaxe miners in a single integration.
...
- 📊 **6+ Sensors** - Hashrate, power, temperature, efficiency, uptime, ASIC count
...
### Manual Installation (Recommended for custom icon)
...
├── const.py                     # Merged constants
├── config_flow.py               # Unified config flow
├── sensor.py                    # Unified sensor platform
...
cards:
  - type: markdown
    content: |
      # ⛏️ Mining Operations
      **Total Hashrate:** {{ states('sensor.total_mining_hashrate') }} GH/s
      **Miners Online:** {{ states.sensor 
         | selectattr('entity_id', 'search', 'miningops.*hashrate')
...
- ✅ Unified config flow
- ✅ Custom icon
- ✅ Support for multiple device types in one integration
